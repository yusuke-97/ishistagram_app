    ...
    <div class="col-sm-8">
        <div class="d-flex align-items-center justify-content-between">
            ...
            <form action="{{ route('unfollow.profile', ['user' => $user->id]) }}" method="POST">
                @csrf
                @method('DELETE')
                <button type="submit" class="profile-link btn-following" style="border: none; color: black; padding: 5px 10px; border-radius: 4px;">フォロー中</button>
            </form>
            ...
            <form action="{{ route('follow.profile', ['user' => $user->id]) }}" method="POST">
                @csrf
                <button type="submit" class="profile-link btn-follow" style="border: none; color: black; padding: 5px 10px; border-radius: 4px;">フォローする</button>
            </form>
            @endif
            @endif
        </div>
